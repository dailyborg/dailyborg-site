...
- **Today's D1 write limit was spent by the old workers before the deploys** (about 105,000 rows by 21:00 UTC), so every write failed until 00:00 UTC. The midnight job ran at 00:02 UTC on 2026-09-10: three of the four counter columns, all nine desk corrections and the discovery deploy succeeded; the first ALTER hit a momentary Cloudflare authorization error (code 7403) and was redone by hand at 00:38 UTC together with the backfill (643 officials). Verified right after: a manual votes pass stored House roll 139 and Senate vote 4, Hank Johnson's counter went from 139 to 140, the 00:05 roster sync wrote nothing for 539 unchanged legislators, and 42 articles published since the restart sit across all seven desks. Sep 10 so far (00:40 UTC): 2,398 rows written, no KV activity at all.
