...
Row shape on insert: `article_type` is `standard` for every published article (`draft` only for drafts); delivery.ts sends everything approved that is not a draft, and the article page shows a badge only for `breaking`; the category lives in the `desk` column. `read_time` is computed from the contentHtml word count at 200 words per minute, minimum 1. The slug is sanitized to a-z, 0-9 and dashes and capped at 120 characters. The model names the event, not the source story, so when that slug already exists the story is skipped as a duplicate of the same event and logged with status `duplicate` (articles.slug is UNIQUE).
