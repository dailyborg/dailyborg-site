        ...
        if (batch.queue === 'enrichment-queue') {
            ...
            for (const message of batch.messages) {
                ...
                try {
                    ...
                    // 2. Fact Extraction via Gemini 1.5 Flash (AIML API)
                    const prompt = `
                        Extract all verifiable facts and promises from this text.
                        Return strictly as JSON array of objects.
                        Format: [{"claim_text": "...", "claim_type": "Fact", "date_said": "YYYY-MM-DD", "context": "..."}]
                        Text: ${enriched_text}
                    `;
                    ...
                    const claims = parsedData.claims || parsedData || []; // Handle array or object wrapping

                    // 3. Image Generation via AIML (Google Gemini Nano Banana 2)
                    let generatedImageUrl = null;
                    try {
                        console.log(`[AUTONOMOUS FEEDER] Generating social image for: ${title}`);
                        const imagePrompt = `High quality editorial political news illustration about: ${title}. Professional, objective, no text.`;
                        ...
                        const imageResponse = await fetch("https://api.aimlapi.com/images/generations", {
                            ...
                            body: JSON.stringify({
                                model: "google/nano-banana-2", // Gemini 3 Flash Image Generation
                                prompt: imagePrompt,
                                n: 1,
                                size: "1024x1024"
                            })
                        ...
                        // We do not fail the whole process if just the image fails
                    }
                    ...
                    for (const claim of claims) {
                        ...
                        // If we generated an image, save it to the context or a new column (assuming context for now)
                        const finalContext = generatedImageUrl
                            ? `${title} | Image: ${generatedImageUrl}`
                            : title;
